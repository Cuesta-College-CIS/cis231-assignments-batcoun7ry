...
public class lab517 {
	public static void main(String[] args) {
        ...
        for (int i = 0; i < numValues; i++){ // loop through words array again with nested for loop to count frequency of each word
            int count = 0;
            for (int j = 0; j < numValues; j++) { // the nested for loop to count frequency
                if (words[j] == words[i]) { // this is the code to detect a match. It works only for the first "self" detection (count goes from 0 to 1), but it does not detect other matches for some reason.
                    count++;
                }
            }
            freq[i] = count; // back in the outer loop, this updates the frequency array with the count value for each "i" index. 
            System.out.println(words[i] + " - " + freq[i]); // prints the word with its frequency, but all frequencies are "1" for some reaosn.
        }
    }
}